...
export default function Login() {
  ...
  const handleSubmit = (event) => {
    ...
    fetch("http://185.229.226.27:3001/user/login", {
      ...
      .catch((err) => {
        alert("Invalid login credentialsssss");
      })
      .finally(() => {});
  };
  ...
  <Grid key={s.name} item xs={12} sm={6} sx={{ mb: 3 }}>
    <TextField
      ...
      id={s.name}
      label={s.label}
      type={s.name === "password" && !showPassword ? "password" : "text"}
      error={!!errors[s.name]}
      autoComplete={s.name}
      helperText={errors[s.name] || ""}
      onChange={s.name === "password" ? handlePasswordChange : handleInputChange}
      InputProps={s.name === "password" ? {
        endAdornment: (
    ...
    />
                  ...
                  <Grid container justifyContent="center"> <Grid item sx={{ mt: 1, mb: 2 }}>
